...
.sidebar{width:var(--sidebar-w);min-width:var(--sidebar-w);max-width:var(--sidebar-w);background:var(--sidebar-bg);display:flex;flex-direction:column;position:fixed;top:0;left:0;height:100vh;z-index:100;border-right:1px solid rgba(45,223,110,.1);overflow-y:auto;overflow-x:hidden}
.sidebar-head{padding:20px 18px 16px;border-bottom:1px solid rgba(45,223,110,.1);display:flex;align-items:center;justify-content:flex-start;gap:10px;width:100%}
.sidebar-logo{width:36px;height:36px;min-width:36px;max-width:36px;flex-shrink:0;border-radius:8px;display:flex;align-items:center;justify-content:center;background:rgba(45,223,110,.08);overflow:hidden}
.sidebar-logo img{width:100%;height:100%;object-fit:contain;mix-blend-mode:screen;display:block}
.sidebar-logo-fallback{font-family:'Syne',sans-serif;color:var(--green);font-weight:800;font-size:13px}
.sidebar-brand{font-family:'Syne',sans-serif;font-size:16px;font-weight:800;color:#fff;letter-spacing:-.3px;line-height:1;white-space:nowrap}
.sidebar-brand em{font-style:normal;color:var(--green)}
.sidebar-tag{font-size:10px;color:rgba(45,223,110,.5);margin-top:3px;white-space:nowrap}
...
.nav-item{display:flex;align-items:center;gap:10px;padding:9px 10px;border-radius:10px;color:rgba(255,255,255,.45);font-size:13px;text-decoration:none;transition:all .18s;position:relative}
...
.modal-body{padding:20px 22px}.modal-footer{display:flex;justify-content:flex-end;gap:10px;padding:14px 22px;border-top:1px solid var(--border)}
    ...
    <section class="content">
      ...
      <div class="card">
        <div class="card-head">
          ...
          <div class="filters">
            <select class="filter-sel" id="filtroEstado" onchange="filtrarTabla()">
            ...
            <select class="filter-sel" id="filtroTipo" onchange="filtrarTabla()">
              <option value="">Todos los tipos</option>
              <?php foreach ($tiposContrato as $tipo): ?>
                <option value="<?= e($tipo['contrato']) ?>"><?= e($tipo['contrato']) ?></option>
              <?php endforeach; ?>
            </select>

            <select class="filter-sel" id="filtroArea" onchange="filtrarTabla()">
              <option value="">Todas las áreas</option>
              <?php foreach ($areas as $area): ?>
                <option value="<?= e($area['nombre_area']) ?>"><?= e($area['nombre_area']) ?></option>
              <?php endforeach; ?>
            </select>
          </div>
        </div>
        ...
        <div class="form-section">
          ...
          <div class="form-grid">
            <div class="form-group span2">
              ...
              <select class="form-select" name="id_trabajador" id="id_trabajador" required onchange="sincronizarTrabajador()">
                <option value="">— Seleccione un trabajador —</option>
                <?php foreach ($trabajadores as $t): ?>
                  <option
                    value="<?= (int)$t['id_trabajador'] ?>"
                    data-area="<?= (int)($t['id_area'] ?? 0) ?>"
                    data-cargo="<?= (int)($t['id_cargo'] ?? 0) ?>"
                  >
                    <?= e(trim(($t['nombres'] ?? '') . ' ' . ($t['apellidos'] ?? ''))) ?> · <?= e($t['numero_documento'] ?? '') ?><?= ((int)($t['estado'] ?? 1) === 0) ? ' · Inactivo' : '' ?>
                  </option>
                <?php endforeach; ?>
              </select>
            </div>
...
function abrirVistaContrato(idx){
  ...
  if (!c) {
    alert('No se encontró la información del contrato.');
    return false;
  }
  ...
  const fila = document.getElementById('row-' + idx);

  if (fila) {
    fila.classList.add('selected');
  }

  const salarioBase = numeroNomina(c.salario_base);
  const auxilio = numeroNomina(c.auxilio_transporte);
  ...
  if (avatar) {
    avatar.className = 'view-avatar ' + (c.avatar_class || 'avatar-blue');
    avatar.textContent = c.iniciales || 'TR';
  }

  setText('verNombre', c.trabajador_nombre || 'Trabajador');

  setText(
    'verSub',
    'ID ' + String(c.id_trabajador || 0).padStart(4, '0') + ' · ' + (c.area || 'Sin área')
  );
...
function filtrarTabla(){
  ...
  document.querySelectorAll('#tablaContratos tbody tr').forEach(tr => {
    ...
    const coincideTexto = !q || (tr.dataset.texto || '').includes(q);
    const coincideEstado = !estado || (tr.dataset.estado || '') === estado;
    const coincideTipo = !tipo || (tr.dataset.tipo || '') === tipo;
    const coincideArea = !area || (tr.dataset.area || '') === area;

    tr.style.display = (coincideTexto && coincideEstado && coincideTipo && coincideArea) ? '' : 'none';
  });
}
...
if (formContrato) {
  formContrato.addEventListener('submit', function(){
    ...
    if ($('salario_base')) {
      $('salario_base').value = limpiarNumero(salarioView ? salarioView.value : '');
    }

    if ($('auxilio_transporte')) {
      $('auxilio_transporte').value = limpiarNumero(auxilioView ? auxilioView.value : '');
    }

    formContrato.querySelectorAll('input, select, textarea').forEach(el => {
      el.disabled = false;
    });
  });
}
...
document.addEventListener('click', function(e){
  const btnCerrarVista = e.target.closest('.js-cerrar-vista');
  if (btnCerrarVista) {
    e.preventDefault();
    cerrarVistaContrato();
    return;
