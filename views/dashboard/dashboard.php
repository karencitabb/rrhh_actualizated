<?php
session_start();
// Proteger: si no está logueado, al login
...
try {
    $total_trabajadores  = $conexion->query("SELECT COUNT(*) FROM trabajadores")->fetchColumn();
    $activos             = $conexion->query("SELECT COUNT(*) FROM trabajadores WHERE estado = 1")->fetchColumn();
    $incidentes_mes      = $conexion->query("SELECT COUNT(*) FROM incidentes WHERE MONTH(fecha) = MONTH(NOW()) AND YEAR(fecha) = YEAR(NOW())")->fetchColumn();
    $capacitaciones      = $conexion->query("SELECT COUNT(*) FROM capacitaciones WHERE estado = 'activa'")->fetchColumn();
    $examenes_pendientes = $conexion->query("SELECT COUNT(*) FROM examenes_medicos WHERE estado = 'pendiente'")->fetchColumn();
    $contratados_mes     = $conexion->query("SELECT COUNT(*) FROM trabajadores WHERE MONTH(fecha_ingreso) = MONTH(NOW()) AND YEAR(fecha_ingreso) = YEAR(NOW())")->fetchColumn();
} catch (Exception $e) {
    // Si alguna tabla no existe aún, simplemente queda en 0
}

$fecha_hoy = strftime('%d de %B de %Y');
// strftime no disponible en todos los PHP modernos, alternativa:
...
:root{
  ...
  --text-muted:#7bb891;
  --text-dim:#3d6b4a;
  --border:rgba(45,223,110,0.13);
  --border-strong:rgba(45,223,110,0.28);
  --sidebar-w:240px;
  --topbar-h:68px;
}

html,body{height:100%;overflow-x:hidden}
body{font-family:'DM Sans',sans-serif;background:var(--g0);color:var(--text);display:flex;flex-direction:column}
...
.sidebar{
  width:var(--sidebar-w);
  background:linear-gradient(180deg,#061009 0%,#04100700 100%);
  background:#050e07;
...
.user-avatar{
  ...
  color:#021a08;flex-shrink:0;
  box-shadow:0 0 14px rgba(45,223,110,0.3);
}
...
.user-role{
  ...
  background:rgba(45,223,110,0.1);
...
.nav-section{
  ...
  color:var(--text-dim);padding:10px 8px 6px;font-weight:600;
}
...
.nav-item:hover{background:var(--green-mist);color:var(--green-pale)}

.nav-item.active{
  background:rgba(45,223,110,0.12);
  color:var(--green);
  font-weight:500;
}
...
.nav-logout svg{width:17px;height:17px;stroke:currentColor;fill:none;stroke-width:1.8}
...
/* ── CONTENT ── */
.content{flex:1;padding:clamp(20px,3vw,36px) clamp(20px,3vw,36px);background:#f0f4f1}

/* welcome banner */
.welcome-banner{
  background:linear-gradient(135deg,rgba(45,223,110,0.07) 0%,rgba(45,223,110,0.02) 100%);
  border:1px solid var(--border);
  border-radius:18px;
  padding:24px 28px;
  margin-bottom:28px;
...
.welcome-text p{font-size:13.5px;color:var(--text-muted);line-height:1.5}
...
.stat-card:hover{
  ...
  box-shadow:0 8px 28px rgba(45,223,110,0.12);
}
...
.stat-label{font-size:12.5px;color:var(--text-muted);font-weight:300}
...
.stat-sub svg{width:12px;height:12px;stroke:currentColor;fill:none;stroke-width:2}

/* color variants */
.green-card .stat-icon{background:#eafaf1;color:#1a9945}
.green-card .stat-num{color:#1a9945}
.green-card .stat-card-glow{display:none}

.blue-card .stat-icon{background:#eff6ff;color:#2563eb}
.blue-card .stat-num{color:#2563eb}
.blue-card .stat-card-glow{display:none}

.red-card .stat-icon{background:#fff1f2;color:#dc2626}
.red-card .stat-num{color:#dc2626}
.red-card .stat-card-glow{display:none}

.yellow-card .stat-icon{background:#fffbeb;color:#d97706}
.yellow-card .stat-num{color:#d97706}
.yellow-card .stat-card-glow{display:none}

.purple-card .stat-icon{background:#f5f3ff;color:#7c3aed}
.purple-card .stat-num{color:#7c3aed}
.purple-card .stat-card-glow{display:none}

.teal-card .stat-icon{background:#f0fdfa;color:#0d9488}
.teal-card .stat-num{color:#0d9488}
.teal-card .stat-card-glow{display:none}
...
.quick-card-icon svg{width:17px;height:17px;stroke:var(--green);fill:none;stroke-width:1.8}
.quick-card-label{font-size:13px;font-weight:500;color:var(--text)}
.quick-card-desc{font-size:11.5px;color:var(--text-dim);line-height:1.4}
...
.quick-card:nth-child(1){animation-delay:.3s}
.quick-card:nth-child(2){animation-delay:.35s}
.quick-card:nth-child(3){animation-delay:.4s}
.quick-card:nth-child(4){animation-delay:.45s}
...
.profile-btn:hover{background:rgba(45,223,110,0.1);border-color:var(--border-strong)}
...
.profile-role{font-size:12px;color:var(--green)}
...
.profile-dropdown{
  ...
  border-radius:16px;
  overflow:hidden;
  box-shadow:0 20px 60px rgba(0,0,0,0.6),0 0 0 1px rgba(45,223,110,0.04);
  display:none;
  animation:ddIn .2s cubic-bezier(.22,1,.36,1) both;
  z-index:200;
}
...
.profile-avatar-lg{
  ...
  color:#021a08;flex-shrink:0;
  box-shadow:0 0 16px rgba(45,223,110,0.3);
}
...
.profile-dd-item svg{width:15px;height:15px;flex-shrink:0;stroke:currentColor;fill:none;stroke-width:1.8}
.profile-dd-item:hover{background:var(--green-mist);color:var(--green-pale)}

.profile-dd-sep{height:1px;background:var(--border);margin:6px 0}

.profile-dd-logout{color:#f87171 !important}
.profile-dd-logout:hover{background:rgba(248,113,113,0.08) !important;color:#fca5a5 !important}
...
<div class="layout">
  ...
  <aside class="sidebar" id="sidebar">
    ...
    <div class="sidebar-user">
      ...
      <div class="user-info">
        <div class="user-name"><?php echo htmlspecialchars($nombres . ' ' . $apellidos); ?></div>
        <span class="user-role"><?php echo $rol_nombre; ?></span>
      </div>
    </div>
    ...
    <nav class="sidebar-nav">
      ...
      <a href="../sst/index.php" class="nav-item">
    ...
    <header class="topbar">
      ...
      <div class="topbar-right">
        ...
        <div class="profile-wrap" id="profileWrap">
          <button class="profile-btn" onclick="toggleProfile()" aria-label="Menú de usuario">
            <div class="profile-avatar"><?php echo $inicial; ?></div>
            <div class="profile-info">
              <span class="profile-name"><?php echo htmlspecialchars($nombres); ?></span>
              <span class="profile-role"><?php echo $rol_nombre; ?></span>
            </div>
            <svg class="profile-chevron" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><polyline points="6 9 12 15 18 9"/></svg>
          </button>
          ...
          <div class="profile-dropdown" id="profileDropdown">
            <div class="profile-dropdown-head">
              ...
              <div>
                <div style="font-size:14px;font-weight:600;color:#fff"><?php echo htmlspecialchars($nombres . ' ' . $apellidos); ?></div>
                <div style="font-size:11.5px;color:var(--green);margin-top:2px"><?php echo $rol_nombre; ?></div>
              </div>
            </div>
            <div class="profile-dropdown-body">
              <a href="../perfil/editar.php" class="profile-dd-item">
              ...
              <a href="../perfil/cambiar-contrasena.php" class="profile-dd-item">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0110 0v4"/></svg>
                Cambiar contraseña
              </a>
              <a href="../perfil/configuracion.php" class="profile-dd-item">
              ...
              <a href="../../logout.php" class="profile-dd-item profile-dd-logout">
    ...
    <div class="content">
      ...
      <div class="welcome-banner">
        <div class="welcome-text">
          <h2 style="font-size:clamp(17px,2vw,23px)"><?php echo $emoji . " " . $saludo . ", "; ?><span style="color:var(--green)"><?php echo htmlspecialchars($nombres); ?></span>! 👋</h2>
          <p>Tu equipo te espera — aquí va el resumen de hoy.</p>
        </div>
      ...
      <div class="stats-grid">
        ...
        <div class="stat-card green-card">
          ...
          <div class="stat-num"><?php echo $total_trabajadores; ?></div>
          <div class="stat-label">Total trabajadores</div>
          <div class="stat-sub">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>
            <?php echo $activos; ?> activos actualmente
          </div>
        </div>
        ...
        <div class="stat-card blue-card">
          ...
          <div class="stat-num"><?php echo $contratados_mes; ?></div>
        ...
        <div class="stat-card red-card">
          ...
          <div class="stat-num"><?php echo $incidentes_mes; ?></div>
          <div class="stat-label">Incidentes SST este mes</div>
          <div class="stat-sub">
        ...
        <div class="stat-card yellow-card">
          ...
          <div class="stat-num"><?php echo $examenes_pendientes; ?></div>
        ...
        <div class="stat-card purple-card">
          ...
          <div class="stat-num"><?php echo $capacitaciones; ?></div>
          <div class="stat-label">Capacitaciones activas</div>
          <div class="stat-sub">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><polyline points="22 7 13.5 15.5 8.5 10.5 2 17"/></svg>
            En progreso
          </div>
        </div>
        ...
        <div class="stat-card teal-card">
          ...
          <div class="stat-num"><?php echo $activos > 0 ? round(($activos/$total_trabajadores)*100) : 0; ?>%</div>
          <div class="stat-label">Cobertura SG-SST</div>
          <div class="stat-sub">
      ...
      <!-- Quick access -->
      <div class="section-title" style="margin-top:28px">Accesos rápidos</div>
      <div class="quick-grid">

        <a href="../trabajadores/index.php" class="quick-card">
          ...
          <div class="quick-card-label">Trabajadores</div>
          <div class="quick-card-desc">Gestionar personal y contratos</div>
        </a>

        <a href="../sst/index.php" class="quick-card">
          <div class="quick-card-icon">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg>
          </div>
          <div class="quick-card-label">SG-SST</div>
          <div class="quick-card-desc">Seguridad y salud en el trabajo</div>
        </a>

        <a href="../capacitaciones/index.php" class="quick-card">
          <div class="quick-card-icon">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M2 3h6a4 4 0 014 4v14a3 3 0 00-3-3H2z"/><path d="M22 3h-6a4 4 0 00-4 4v14a3 3 0 013-3h7z"/></svg>
          </div>
          <div class="quick-card-label">Capacitaciones</div>
          <div class="quick-card-desc">Formación y entrenamientos</div>
        </a>

        <a href="../examenes/index.php" class="quick-card">
          <div class="quick-card-icon">
          ...
          <div class="quick-card-label">Exámenes Médicos</div>
          <div class="quick-card-desc">Control de aptitud laboral</div>
        </a>
...
(function(){
  ...
  function P(){
    ...
    this.alpha=Math.random()*.35+.04;
    this.col=COLORS[Math.floor(Math.random()*COLORS.length)];
    this.pulse=Math.random()*Math.PI*2;this.ps=.006+Math.random()*.01;
  }
  ...
  function init(){const n=Math.floor(W*H/18000);particles=[];for(let i=0;i<n;i++)particles.push(new P())}
  function drawLines(){
    const max=90;
    for(let i=0;i<particles.length;i++)
      for(let j=i+1;j<particles.length;j++){
        ...
        if(d<max){ctx.beginPath();ctx.moveTo(particles[i].x,particles[i].y);ctx.lineTo(particles[j].x,particles[j].y);ctx.strokeStyle='rgba(45,223,110,'+(0.035*(1-d/max))+')';ctx.lineWidth=.5;ctx.stroke()}
      }
  }
